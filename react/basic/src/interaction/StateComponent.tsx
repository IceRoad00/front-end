...
export default function StateComponent() {
    ...
    const onCountAddHandler = () => {
        ...
        // 상태변수는 반드시 상태변경함수로 변경해야 리랜더링됨
        // count++;
        // console.log(count);

        // 상태변경 함수를 통해서 함수를 변경한다고 바로 적용되지 않음
        // 리랜더링된 후 상태변경 함수가 적용됨
        // setCount(count + 1);
        // setCount(count + 1);
        // setCount(count + 1);

        // 상태변경 함수에 콜백 함수를 전달하면 해당 콜백 함수는 상태 변경 작업을 누적해서 실행함
        // setCount((count) => count + 1);
        // setCount((count) => count + 1);
        // setCount((count) => count + 1);

        // 변경된 상태를 사용하고 싶을 때 해결방법, 임시 변수를 사용하여 간접 산용
    ...
    const onAddListHandler = () => {
        ...
        // 상태가 변경된 것으로 인식하게 하고 싶으면 새로운 배열 혹은 객체를 생성하여 상태를 변경해야함(일반적으로 복사해서 변경)
