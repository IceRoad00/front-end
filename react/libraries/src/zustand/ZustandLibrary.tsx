...
// - 상태 관리 라이브러리 중 Redux 라이브러리가 가장 많이 사용됨
// - Redux의 고질적인 문제점으로 복잡한 코드 구조와 높은 학습 곡선을 요구
// - Zustand는 단순한 코드 구조와 학습 곡선이 매우 낮음 - useState 사용 수준의 학습 곡선을 요구
// - Redux, MobX와 같은 타 상태관리 라이브러리들에 비해 가벼움 - 빌드할 때 패키징 속도 향상 / 빌드 후 번들의 크기가 작음
...
// - create 함수에 매개변수로 set 인자를 받는 콜백 함수를 전달해야함
// - 매개변수로 전달한 콜백 함수는 store 객체를 반환해야함
// - store 객체는 상태,  store 객체를 변경하는 함수 포함

// - typescript에서 zustand의 create 함수를 사용할 때는 create 함수의 제너릭으로 스토어의 타입을 지정해야함
